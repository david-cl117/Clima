...
function cargar(){
            ...
            if(data == "No ha iniciado sesion"){
                ...
                .done(function(padre){
                    ...
                    if(padre!=""){
                        ...
                        //activamos los botones para agregar notas y planes.
                        document.getElementById("AddN").disabled = false;
                        document.getElementById("AddP").disabled = false;

                        papa = eval(padre);
                        //Sacamos nombre y apellido del padre
                        userP = papa[0];
                        nombreP = papa[1];
                        apPadre = papa[2];

                        //sacamos usuario del terapeuta
                        ...
                        alert ("No se encontro al padre, por favor registre al padre o tutor del alumno.");

                        //Boton para registrar padre
                        var BCrearP = document.createElement("button");
                        BCrearP.appendChild(document.createTextNode("Registrar Padre"));
                        document.getElementById('transparente').appendChild(BCrearP);
                        BCrearP.className = "regisP";

                        //Estilo del boton
                        BCrearP.style.position = "absolute";
                        BCrearP.style.bottom = "30px";
                        BCrearP.style.left = "475px";
                        BCrearP.style.width = "150px";
                        BCrearP.style.height = "40px";
                        BCrearP.style.border = "none";
                        BCrearP.style.color = "white";
                        BCrearP.style.backgroundColor = "rgba(235, 64, 52, 0.8)";
                        BCrearP.style.fontSize = "17px";
                        BCrearP.style.borderRadius = "15px";
                        BCrearP.style.outline = "none";

                        //Hover boton
                        $(".regisP").hover(function(){
                        ...
                        //desactivamos los botones para agregar notas y planes.
                        document.getElementById("AddN").disabled = true;
                        document.getElementById("AddP").disabled = true;

                        //Click registrar padre
    ...
    //Funciones para cargar notas, mensajes y observaciones.
    loadNotes();
    loadPlans();
    loadObser();
            ...
            var notas = eval("["+notesarray+"]");
             ...
             for(i=0; i<notas.length; i++){
                for(j=0; j<notas[i].length; j++){
                    nombre_nota = (notas[i][0]); //nombre de la nota
                    fecha_nota = (notas[i][1]); //fecha de la nota
                    id_nota = (notas[i][2]); //id de la nota
                }    
            ...
            var planes = eval("["+planesarray+"]");
             ...
             for(i=0; i<planes.length; i++){
                ...
                $("#plan"+id_plan).click(function(e){
                    ...
                    //Para obtener el link de los planes
...
function loadMessages(mensajes){
    ...
    var fecha_men;
     ...
     for(i=0; i<mensajes.length; i++){
        ...
        var hTe = document.createElement("p");
        var textTer = document.createTextNode(nombreT + " " + apT + " dice:");
        hTe.appendChild(textTer);
        hTe.className = "nombreTer";
        ...
        //Estilo hA
        hTe.style.position = "relative";
        hTe.style.top = "0px";
        hTe.style.fontSize = "11px";
        hTe.style.left = "5px";
        hTe.style.marginTop = "10px";
        hTe.style.marginBottom = "0px";
        ...
        if(transmisor == usuarioT){
            ...
            document.getElementById("chat").appendChild(hTe);
            
        }

        //Si el mensaje es del padre
...
//funcion para cerrar menu de notas
function cerrarmenu(){
...
//Presionar enter para publicar observacion.
function enterobs(e){
    if(e.which == 13 && !e.shiftKey){
        $('#pubob').click();
        e.preventDefault();
    ...
    if(files != null){
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data == "formatError"){
                    alert("Sólo puede subir archivos formato .pdf o en word.");
                }
...
//Funcion para abrir popup para registrar padre
function abrirPopAP(){
    document.getElementById("popupPa").style.display = "block";
}

//Funcion para cerrar popup para registrar padre
function cerrarPopAP(){
    document.getElementById("popupPa").style.display = "none";

    document.getElementById("NamePadre").value = "";
    document.getElementById("Paterno").value = "";
    document.getElementById("Materno").value = "";
}

//Funcion para registrar padre
function agregarPa(){

    //Leemos datos del padre
    var nombrePadre = document.getElementById("NamePadre").value;
    var apellido_P = document.getElementById("Paterno").value;
    var apellido_M = document.getElementById("Materno").value;

    //Sacamos iniciales del alumno para crear usuario del padre
    var inicialN = nombre.charAt(0);
    var inicialP = apP.charAt(0);
    var inicialM = apM.charAt(0);
    var iniciales = inicialN + inicialP + inicialM;

    //Usuario
    var usuarioPadre = iniciales+"-"+idA;

    //PARA OBTENER fecha de nacimiento del alumno (contraseña de padre)
    $.post("/Clima/php/papas.php", {id: idA})
    .done(function(fechaNac){
        if(fechaNac!=""){
            ...
            $.post("/Clima/php/registraPapa.php", {usr: usuarioPadre, name: nombrePadre, apP: apellido_P, apM: apellido_M, pass: contraP, idA: idA});
            alert("El padre o tutor se ha registrado, su nombre de usuario es: " + usuarioPadre + " y la contraseña es la fecha de nacimiento de su hijo en el siguiente formato: DD-MM-AAAA" );
